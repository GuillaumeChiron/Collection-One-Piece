...
const animatedPossede = useTransition(computed(() => collectionStore.possedeCount), {
  duration: 700,
...
<template>
  <header class="header-hairline border-b border-ink-border bg-ink-surface/60 px-4 py-8 text-center shadow-panel sm:px-8">
    <h1 class="font-display text-4xl tracking-wide text-gold sm:text-5xl">Ma collection One Piece</h1>
    ...
    <div class="mx-auto mt-4 h-3 w-full max-w-xl overflow-hidden rounded-full bg-ink-border shadow-glow-gold">
      <div
    ...
    <div class="mx-auto mt-4 flex max-w-xl flex-wrap justify-center gap-2">
      ...
      <span v-if="reserves > 0" class="rounded-full border border-crimson/40 bg-crimson/10 px-3 py-1 text-xs text-crimson-light">
...
.header-hairline::after {
  ...
  inset: auto 0 -1px 0;
  height: 1px;
  background: linear-gradient(90deg, transparent, color-mix(in oklab, var(--color-gold) 60%, transparent), transparent);
}
